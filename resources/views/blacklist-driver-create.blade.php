...
<head>
  ...
  <title>Blacklist Driver - SIGMA</title>
  ...
  <div class="min-h-screen">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-[258px] bg-[#0b2553] text-white transition-all duration-300">
      ...
      <nav class="h-[calc(100vh-78px)] overflow-y-auto px-[10px] pb-6 pt-[10px] sidebar-scroll">
        ...
        <a class="nav-item active" href="{{ route('blacklist-driver.index') }}"><i data-lucide="ban"></i><span>Blacklist Driver</span></a>
    ...
    <main id="main" class="ml-[258px] min-h-screen pt-[64px] transition-all duration-300">
      <div class="role-access-page min-h-[calc(100vh-64px)] bg-[#f4f7fb] px-[26px] pb-8 pt-[25px]">
        <div class="mx-auto max-w-[920px]">
          <a href="{{ route('blacklist-driver.index') }}" class="mb-4 inline-flex items-center gap-2 text-[11px] font-bold text-[#647b9d]"><i data-lucide="arrow-left" class="h-4 w-4"></i>Kembali ke Blacklist Driver</a>
          <div class="role-access-card rounded-[14px] border border-[#dce5f0] bg-white p-6 shadow-sm">
            <div class="border-b border-[#e6ecf4] pb-5">
              <h1 class="role-page-title text-[22px] font-extrabold text-[#071d43]">Tambah Blacklist Driver</h1>
              <p class="role-page-subtitle mt-1 text-[11px] text-[#7c90b1]">Tambahkan data driver baru ke Blacklist Driver.</p>
            </div>
            @if ($errors->any())
              <div class="mt-5 rounded-[9px] border border-red-200 bg-red-50 p-3 text-[11px] text-red-700">{{ $errors->first() }}</div>
            @endif
            <form method="POST" action="{{ route('blacklist-driver.store') }}" class="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
              @csrf
              
              <label class="text-[10px] font-extrabold text-[#526987]">Nama
                <input name="name" value="{{ old('name', '') }}" class="form-control mt-2 h-[42px] w-full rounded-[8px] border border-[#d7e0ec] bg-white px-3 text-[11px]" required>
              </label>
              <label class="text-[10px] font-extrabold text-[#526987]">Identitas
                <input name="identity" value="{{ old('identity', '') }}" class="form-control mt-2 h-[42px] w-full rounded-[8px] border border-[#d7e0ec] bg-white px-3 text-[11px]" required>
              </label>
              <label class="text-[10px] font-extrabold text-[#526987]">Role / Kategori
                <select id="accessRole" name="role" class="form-control mt-2 h-[42px] w-full rounded-[8px] border border-[#d7e0ec] bg-white px-3 text-[11px]" required>
                  @foreach($roles as $roleOption)<option value="{{ $roleOption }}" @selected(old('role', 'User') === $roleOption)>{{ $roleOption }}</option>@endforeach
                </select>
              </label>
              <label class="text-[10px] font-extrabold text-[#526987]">Gate
                <select id="accessGate" name="gate" class="form-control mt-2 h-[42px] w-full rounded-[8px] border border-[#d7e0ec] bg-white px-3 text-[11px]" required>
                  @foreach(['All Gate','Gate 1','Gate 2','Gate 3'] as $gate)<option value="{{ $gate }}" @selected(old('gate', 'Gate 1') === $gate)>{{ $gate }}</option>@endforeach
                </select>
              </label>
              <label class="text-[10px] font-extrabold text-[#526987] md:col-span-2">Status
                <select name="status" class="form-control mt-2 h-[42px] w-full rounded-[8px] border border-[#d7e0ec] bg-white px-3 text-[11px]" required>
                  ...
                  <option value="Blocked" @selected(old('status', 'Available') === 'Blocked')>Blocked</option>
                </select>
              </label>
              <div class="flex justify-end gap-3 border-t border-[#e6ecf4] pt-5 md:col-span-2">
                <a href="{{ route('blacklist-driver.index') }}" class="inline-flex h-[40px] items-center rounded-[8px] border border-[#d5dfec] px-5 text-[11px] font-extrabold text-[#526987]">Batal</a>
                <button type="submit" class="inline-flex h-[40px] items-center gap-2 rounded-[8px] bg-[#246edb] px-5 text-[11px] font-extrabold text-white hover:bg-[#1e62c6]"><i data-lucide="save" class="h-4 w-4"></i>Simpan Data</button>
              </div>
            </form>
          </div>
        </div>
