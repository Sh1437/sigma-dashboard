...
class SuspendDriverController extends Controller
{
    ...
    private const STATUSES = ['Available', 'Pending'];

    private function baseDrivers(): array
    {
        return [
            ['name'=>'Ahmad','identity'=>'DRV-001','role'=>'Driver','gate'=>'Gate 2','status'=>'Available'],
            ['name'=>'Siti','identity'=>'DRV-002','role'=>'Driver','gate'=>'Gate 1','status'=>'Pending'],
        ];
    }

    private function drivers(Request $request): array
    {
        $drivers = array_merge($request->session()->get('sigma_suspend_driver_users', []), $this->baseDrivers());
        $updates = $request->session()->get('sigma_suspend_driver_updates', []);
        $deleted = $request->session()->get('sigma_suspend_driver_deleted', []);
    ...
    public function index(Request $request): View|RedirectResponse
    {
        ...
        return view('suspend-driver', array_merge($this->authViewData($request), ['users'=>$users->values()->all(),'totalUsers'=>$totalUsers,'roles'=>$roles,'filters'=>compact('search','status','role')]));
    }

    public function create(Request $request): View|RedirectResponse
    {
        if (! $request->session()->has('sigma_auth')) return redirect()->route('login');
        return view('suspend-driver-create', array_merge($this->authViewData($request), ['roles'=>self::ROLES,'gates'=>self::GATES]));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $items=$request->session()->get('sigma_suspend_driver_users',[]); array_unshift($items,$data); $request->session()->put('sigma_suspend_driver_users',$items);
        return redirect()->route('suspend-driver.index')->with('success','Data Suspend Driver berhasil ditambahkan.');
    }

    public function edit(Request $request, string $identity): View|RedirectResponse
    {
        if (! $request->session()->has('sigma_auth')) return redirect()->route('login');
        $user=collect($this->drivers($request))->firstWhere('identity',$identity); abort_if(! $user,404,'Data Suspend Driver tidak ditemukan.');
        return view('suspend-driver-edit', array_merge($this->authViewData($request), ['roles'=>self::ROLES,'gates'=>self::GATES,'user'=>$user]));
    }

    public function update(Request $request, string $identity): RedirectResponse
    {
        if (! $request->session()->has('sigma_auth')) return redirect()->route('login');
        $existing=collect($this->drivers($request))->firstWhere('identity',$identity); abort_if(! $existing,404,'Data Suspend Driver tidak ditemukan.');
        $data=$this->validateData($request);
        ...
        $updates=$request->session()->get('sigma_suspend_driver_updates',[]); $updates[$identity]=$data; $request->session()->put('sigma_suspend_driver_updates',$updates);
        return redirect()->route('suspend-driver.index')->with('success','Data Suspend Driver berhasil diubah.');
    }
    ...
    public function destroy(Request $request, string $identity): RedirectResponse
    {
        ...
        $items = array_values(array_filter($request->session()->get('sigma_suspend_driver_users', []), fn ($u) => $u['identity'] !== $identity));
        $request->session()->put('sigma_suspend_driver_users', $items);
        $updates = $request->session()->get('sigma_suspend_driver_updates', []); unset($updates[$identity]); $request->session()->put('sigma_suspend_driver_updates', $updates);
        $deleted = $request->session()->get('sigma_suspend_driver_deleted', []); if (! in_array($identity, $deleted, true)) $deleted[] = $identity; $request->session()->put('sigma_suspend_driver_deleted', $deleted);
        return redirect()->route('suspend-driver.index')->with('success', 'Data Suspend Driver berhasil dihapus.');
    }
