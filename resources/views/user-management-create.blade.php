        ...
        <div class="mx-auto max-w-[920px]">
          ...
          <div class="role-access-card rounded-[14px] border border-[#dce5f0] bg-white p-6 shadow-sm">
            <div class="border-b border-[#e6ecf4] pb-5">
              <h1 class="role-page-title text-[22px] font-extrabold text-[#071d43]">Tambah User</h1>
              <p class="role-page-subtitle mt-1 text-[11px] text-[#7c90b1]">Tambahkan data user baru ke User Management.</p>
            </div>
            @if ($errors->any())
              <div class="mt-5 rounded-[9px] border border-red-200 bg-red-50 p-3 text-[11px] text-red-700">{{ $errors->first() }}</div>
            @endif
            <form method="POST" action="{{ route('user-management.store') }}" class="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
              @csrf
