    ...
    <main id="main" class="ml-[258px] min-h-screen pt-[64px] transition-all duration-300">
      <div class="role-access-page min-h-[calc(100vh-64px)] bg-[#f4f7fb] px-[26px] pb-4 pt-[25px]">
        <div class="flex items-start justify-between gap-4">
          <div>
            <h1 class="role-page-title text-[23px] font-extrabold tracking-[-.03em] text-[#071d43]">User Management</h1>
            <p class="role-page-subtitle mt-1 text-[11px] font-medium text-[#7c90b1]">Kelola data user dan status pengguna SIGMA.</p>
          </div>
          <a href="{{ route('user-management.create') }}" class="inline-flex h-[42px] items-center gap-2 rounded-[9px] bg-[#246edb] px-5 text-[11px] font-extrabold text-white shadow-sm transition hover:bg-[#1e62c6]">
        ...
        <section class="mt-[20px] overflow-hidden rounded-[12px] border border-[#dce5f0] bg-white shadow-[0_1px_2px_rgba(20,45,82,.03)] role-access-card">
          <form method="GET" action="{{ route('user-management.index') }}" class="flex flex-wrap items-center gap-2 border-b border-[#dce5f0] p-[12px] role-filter-form">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      document.querySelectorAll('.role-detail-btn').forEach(btn => btn.addEventListener('click', () => {
        ...
        </div><a href="{{ url('/user-management') }}/${encodeURIComponent(u.identity)}/edit" class="mt-4 inline-flex h-[38px] w-full items-center justify-center gap-2 rounded-[8px] bg-[#246edb] px-4 text-[11px] font-extrabold text-white hover:bg-[#1e62c6]"><span>Ubah Data</span></a><form method="POST" action="{{ url('/user-management') }}/${encodeURIComponent(u.identity)}" class="mt-2" onsubmit="return confirm('Yakin ingin menghapus data ' + u.name + ' (' + u.identity + ')? Data yang dihapus tidak dapat dikembalikan.');">{{ csrf_field() }}{{ method_field('DELETE') }}<button type="submit" class="inline-flex h-[38px] w-full items-center justify-center gap-2 rounded-[8px] border border-red-200 bg-red-50 px-4 text-[11px] font-extrabold text-red-600 hover:bg-red-100"><span>Hapus Data</span></button></form>`;
        open();
      }));
    });
  </script>
